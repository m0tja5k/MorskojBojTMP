...
QByteArray handleStartGame(const QString &data, MyTcpServer *server) {
    ...
    if (server->getPlayerCount() == 2) {
        ...
        server->sendMessageToUser(nickname, createJsonResponse("game_ready", "success", "Game ready to start"));
        server->sendMessageToUser(opponent, createJsonResponse("game_ready", "success", "Game ready to start"));
    }
