...
{
    // Проверка корректности расстановки кораблей
    ...
    for (int row = 0; row < 10; ++row) {
        for (int col = 0; col < 10; ++col) { // Исправлено: row < 10 → col < 10
            if (playerField[row][col] == Ship && !visited[row][col]) {
                int length = 1;
                bool horizontal = false;
...
void GameWindow::clearFields()
{
    ...
    for (int i = 0; i < playerCells.size(); ++i) { // Исправлено: проверка на размер
