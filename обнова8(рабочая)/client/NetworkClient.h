...
public:
    ...
    void queueShip(int gameId, int x, int y, int size, bool isHorizontal); // Новый метод для очереди
...
signals:
    ...
    void allShipsPlaced(); // Новый сигнал
...
public slots:
    ...
    void sendNextShip(); // Новый слот для отправки следующего корабля
...
private:
    ...
    int currentGameId = -1;
    QQueue<Ship> shipQueue; // Очередь кораблей
};
