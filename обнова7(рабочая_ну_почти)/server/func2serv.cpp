    ...
    if (type == "register") {
        ...
        if (doc.isObject()) {
            ...
            if (obj["status"].toString() == "success") {
                ...
                obj["nickname"] = nickname; // Добавляем никнейм в ответ
        ...
        // Запрос обрабатывается в slotServerRead, здесь просто возвращаем подтверждение
...
QByteArray handleStartGame(const QString &data, MyTcpServer *server) {
    ...
    if (server->getPlayerCount() == 2) {
        ...
        if (gameId != -1) {
            server->currentGameId = gameId; // Сохраняем ID игры
...
QByteArray handlePlaceShip(const QString &data, MyTcpServer *server) {
    ...
    QJsonObject jsonObj = doc.object();
    if (!jsonObj.contains("nickname") || !jsonObj.contains("game_id") || !jsonObj.contains("x") || !jsonObj.contains("y") || !jsonObj.contains("size") || !jsonObj.contains("is_horizontal")) {
...
QByteArray handleMakeMove(const QString &data, MyTcpServer *server) {
    ...
    // Уведомляем оппонента о результате хода
    QString opponent = server->getOpponent(nickname);
