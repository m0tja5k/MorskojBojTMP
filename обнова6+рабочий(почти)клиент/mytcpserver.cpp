...
void MyTcpServer::slotServerRead()
{
    ...
    if (clientSocket) {
        ...
        qDebug() << "Received raw request:" << requestData.toHex(); // Отладка сырых данных
        qDebug() << "Received input (parsed):" << request; // Отладка парсинга
        // Парсим запрос
        ...
        if (doc.isObject()) {
            ...
            if (type == "register" || type == "login") {
                ...
                if (!nickname.isEmpty() && players.contains(nickname)) {
                    ...
                    response = createJsonResponse("ready_to_battle", "success", "Ready status received"); // Ответ для каждого запроса
                    if (readyPlayers.size() == 2 && currentGameId != -1) {
                        ...
                        db->updateTurn(currentGameId, player1); // Первый ход у player1
                        ...
                        // Отправка game_start всем клиентам
                        for (const QString &player : mClients.keys()) {
                            ...
                            sendMessageToUser(player, startResponse);
                        }
                    }
                }
            }
...
void MyTcpServer::resetGame()
{
    ...
    players.clear();
    readyPlayers.clear();
    currentGameId = -1; // Сбрасываем ID игры
    qDebug() << "Game reset.";
}
