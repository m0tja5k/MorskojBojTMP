...
{
    while (m_socket->canReadLine()) { // Читаем построчно
        const QByteArray data = m_socket->readLine().trimmed();
        qDebug() << "Received raw data from server:" << data; // Выводим сырые данные в консоль
        ...
        qDebug() << "Parsed message type:" << type; // Выводим тип сообщения

        if (type == "register") {
            if (json["status"] == "success") {
                qDebug() << "Registration successful for nickname:" << json["nickname"].toString();
            ...
            if (json["status"] == "success") {
                qDebug() << "Login successful for nickname:" << json["nickname"].toString();
            ...
            currentGameId = gameId; // Сохраняем game_id
            ...
            if (json["status"] == "success") {
                ...
                qDebug() << "Ready to battle failed. Reason:" << json["message"].toString();
...
void NetworkClient::sendMessage(const QString& message)
{
    ...
    if (m_socket->state() == QAbstractSocket::ConnectedState) {
        QByteArray data = message.toUtf8() + "\r\n"; // Добавляем \r\n для корректного завершения строки
